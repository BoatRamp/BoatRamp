...
fn decode_string(buf: &[u8]) -> Result<(Cow<'_, [u8]>, usize), DecoderError> {
    ...
    if buf[0] & 128 == 128 {
        ...
        // Huffman coding used: pass the raw octets to the Huffman decoder and return its
        // result. `HuffmanDecoder::new()` rebuilds a 257-symbol nested HashMap, which
        // profiled as the top per-request cost when constructed for every Huffman string;
        // the table is immutable, so build one process-wide decoder and share it.
        static HUFFMAN: std::sync::OnceLock<HuffmanDecoder> = std::sync::OnceLock::new();
        let decoder = HUFFMAN.get_or_init(HuffmanDecoder::new);
